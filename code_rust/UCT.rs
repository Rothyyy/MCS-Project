...
use crate::models::conjectures::extremal::{State, Move};
...
pub fn playout(mut st: State, heuristic_w : f64) -> State{
    ...
    while !st.terminal() {
        ...
        if heuristic_w != 0.0 {
            ...
            for &m in &moves {
                let mut choice =  heuristic_w*st.heuristic(m);
                if State::CONSIDER_VARIANCE_DEGRE {
                    choice = choice + st.bonus_smallest_degre(m);
                }
                weights.push(choice);
            }
        ...
        if State::CONSIDER_VARIANCE_DEGRE {
            let mut weights = Vec::new();
            for &m in &moves {
                weights.push(st.bonus_smallest_degre(m));
            }
            i = softmaxChoice(weights);
        }
        let mv = moves[i];
